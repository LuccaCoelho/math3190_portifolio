...
# Path to where you want the database FILE saved
conn <- dbConnect(RSQLite::SQLite(), "/home/lucca-coelho/school/math_3190_hw2_data/flights_db.sqlite")

files <- list.files("/home/lucca-coelho/school/math_3190_hw2_data", pattern = "\\.csv$", full.names = TRUE)
...
```{r}
# 1
dbGetQuery(conn, "SELECT AVG(DISTANCE) AS avg_distance FROM flights;")


# 2
dbGetQuery(conn, "SELECT MIN(DISTANCE) AS min_distance FROM flights;")

# 3
dbGetQuery(conn, "SELECT MAX(DISTANCE) AS max_distance FROM flights;")
...
# 5
dbGetQuery(conn, "
SELECT DISTINCT ORIGIN_AIRPORT, DESTINATION_AIRPORT, DISTANCE
FROM flights
...
# 7
dbGetQuery(conn, "
SELECT a1.AIRPORT AS origin_airport_name,
       a2.AIRPORT AS dest_airport_name,
       f.DISTANCE
FROM flights f
INNER JOIN airports a1 ON f.ORIGIN_AIRPORT = a1.IATA_CODE
INNER JOIN airports a2 ON f.DESTINATION_AIRPORT = a2.IATA_CODE
WHERE f.DISTANCE = (SELECT MIN(DISTANCE) FROM flights);
")

# 8
dbGetQuery(conn, "
SELECT DISTINCT a1.AIRPORT AS origin_airport_name,
       a2.AIRPORT AS dest_airport_name,
       f.DISTANCE
FROM flights f
INNER JOIN airports a1 ON f.ORIGIN_AIRPORT = a1.IATA_CODE
INNER JOIN airports a2 ON f.DESTINATION_AIRPORT = a2.IATA_CODE
WHERE f.DISTANCE = (SELECT MAX(DISTANCE) FROM flights)
  AND f.ORIGIN_AIRPORT NOT LIKE '1%'
  AND f.DESTINATION_AIRPORT NOT LIKE '1%';
")
...
# 1
delays_tbl <- dbGetQuery(conn, "
SELECT f.ARRIVAL_DELAY, al.AIRLINE
FROM flights f
...
# 2
ggplot(delays_tbl, aes(x = AIRLINE, y = ARRIVAL_DELAY)) +
  geom_boxplot() +
  scale_y_log10() +
...
# 1
flights_tbl  <- tbl(conn, "flights")
airlines_tbl <- tbl(conn, "airlines")
airports_tbl <- tbl(conn, "airports")

# 2
results <- flights_tbl |>
  inner_join(airlines_tbl, by = c("AIRLINE" = "IATA_CODE")) |>
  inner_join(airports_tbl, by = c("ORIGIN_AIRPORT" = "IATA_CODE")) |>
  group_by(AIRLINE) |>
  summarise(
  ...
  arrange(desc(sd_elapsed)) |>
  collect()

# 3
ggplot(results, aes(x = AIRLINE, y = sd_elapsed)) +
  geom_col() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
    x = "Airline",
    y = "Standard Deviation of Elapsed Time",
    title = "Flight Time Variability by Airline"
  )
