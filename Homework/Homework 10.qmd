...
wine_df = ts(wine, deltat = 1/12, start = 1980) |>
  as_tsibble() |>
  rename(date = index, wine_sales = value)

ggplot(wine_df, aes(x = date, y = wine_sales)) +
  geom_line(color = "steelblue") +
  geom_point(color = "steelblue", size = 0.8) +
  scale_x_yearmonth(date_breaks = "1 year", date_labels = "%Y") +
  labs(title = "Australian Red Wine Sales (Jan 1980 – Oct 1991)",
       x = "Year", y = "Sales") +
  theme_minimal()
```
...
```{r}
wine_train = wine_df |> slice_head(n = nrow(wine_df) - 34)
wine_test  = wine_df |> slice_tail(n = 34)
```
...
gg_season(wine_train, wine_sales) +
  labs(title = "Seasonal Plot of Wine Sales")
```

- There is a slight pattern where sales seems to increase from January and hit a high spike around July/August then shoot down for the end of the year.
...
adf.test(wine_train$wine_sales, k = 12)
```

- Since p-value is 0.7387, we don't have enough evidence that the time series in stationary. 
...
```{r}
wine_diff = diff(wine_train$wine_sales)

plot(wine_diff, type = "l", main = "Differenced Wine Sales", ylab = "Differenced Sales")

adf.test(wine_diff, k = 12)
```

- Since p-value is 0.01, we have enough evidence to suggest that the differenced series is stationary.
...
wine_fc = forecast::forecast(wine_arima, h = 34)

autoplot(wine_fc) +
  labs(title = "Wine Sales Forecast (34 Months)")
```
...
```{r}
fc_df = data.frame(
  index    = wine_test$date,
  forecast = wine_fc$mean,
  lo95     = wine_fc$lower[, 2],
  hi95     = wine_fc$upper[, 2],
  actual   = wine_test$wine_sales
)

ggplot(fc_df, aes(x = index)) +
  geom_ribbon(aes(ymin = lo95, ymax = hi95), fill = "steelblue", alpha = 0.2) +
  geom_line(aes(y = forecast), color = "steelblue", linewidth = 0.8) +
  geom_point(aes(y = actual), color = "firebrick", size = 1.2) +
  geom_line(aes(y = actual), color = "firebrick", linewidth = 0.6) +
  scale_x_yearmonth(date_breaks = "6 months", date_labels = "%b %Y") +
  labs(title = "Actual vs. Forecast (1989–1991)",
       x = "Date", y = "Sales",
       caption = "Red = actual, Blue = forecast, Shaded = 95% CI") +
  theme_minimal()
```

- Yes there are many points where the actual value is outside of the 95% CI. 
...
```{r}
checkresiduals(wine_arima)
```

- Ljung-Box test: Since p-value is 0.3514, we don't have enough evidence of autocorrelation.

- Residual plot: The residuals are all around 0 with no obvious trend, but with time the variation seems to increase.
- ACF Plot: All lags falls within the 95% bounds.
- Histogram: Don't look super nice, but the distribution are pretty much bell-shaped.

- Overall: The normality and independence assumptions seems to be met
